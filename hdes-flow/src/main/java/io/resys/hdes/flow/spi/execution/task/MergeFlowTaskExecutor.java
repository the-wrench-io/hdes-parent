...
public class MergeFlowTaskExecutor implements FlowTaskExecutorFactory.Executor {

  @Override
  public ExecutionTask apply(FlowAst.Task task, FlowExecution execution) {
    boolean isOpen = execution.getValue().stream()
        .filter(e -> task.getPrevious().getId().equals(e.getId()))
        .filter(e -> e.getStatus().equals(ExecutionStatus.OPEN))
        .findFirst().isPresent();

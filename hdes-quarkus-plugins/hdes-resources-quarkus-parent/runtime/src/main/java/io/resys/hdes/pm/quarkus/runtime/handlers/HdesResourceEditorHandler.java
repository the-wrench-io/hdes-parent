...
import io.resys.hdes.resource.editor.spi.support.ImmutableStatusMessage;
import io.resys.hdes.resource.editor.spi.support.RepoAssert;
import io.resys.hdes.resource.editor.spi.support.RepoAssert.StatusMessage;
...
public abstract class HdesResourceEditorHandler implements Handler<RoutingContext> {
  private static final Logger LOGGER = LoggerFactory.getLogger(HdesResourceEditorHandler.class);
  private final CurrentIdentityAssociation currentIdentityAssociation;
  private final CurrentVertxRequest currentVertxRequest;
  
  public HdesResourceEditorHandler(
  ...
  protected abstract void handleResource(RoutingContext event, HttpServerResponse response, HdesResourceEditorContext ctx);
  ...
  public void handle(RoutingContext event) {
    ...
    if (requestContext.isActive()) {
      ...
      HdesResourceEditorContext ctx = CDI.current().select(HdesResourceEditorContext.class).get();
      try {
  ...
  public static void catch404(String id, HdesResourceEditorContext ctx, HttpServerResponse response) {
  ...
  public static void catch422(Exception e, HdesResourceEditorContext ctx, HttpServerResponse response) {

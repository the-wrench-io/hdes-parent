...
import io.resys.hdes.projdb.spi.builders.ObjectsQuery.Query;
...
public abstract class MongoQueryTemplate<Q, T> implements Query<Q, T> {
  ...
  @Value.Immutable
  public interface QueryResultWithAccess<T> {
    T getValue();
    Bson getFilter();
  }
  ...
  public <W> W unwrap(Class<W> type) {
    if(type == MongoQueryTemplate.class) {
      return (W) this;
    }
    RepoAssert.fail(() -> "unknown wrapper: " + type + "!");
    return null;
  }

...
public class JexlExpressionTest {
  ...
  public void stringIn() throws IOException {
    ...
    Assertions.assertEquals(true, build("task1 == 0").getOperation().apply(new MapContext(tasks)));
    Assertions.assertEquals(false, build("task1 == 1").getOperation().apply(new MapContext(tasks)));
    Assertions.assertEquals(true, build("task2 == 3").getOperation().apply(new MapContext(tasks)));
  }

  public DataTypeService.Expression build(String src) {
    return new LogginOperation(expressionFactory.builder().src(src).srcType(ExpressionSourceType.JEXL).valueType(ValueType.OBJECT).build());
  }
